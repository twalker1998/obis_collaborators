    ...
    return new Promise((resolve, reject) => {
      for (let r of this.results) {
        ...
        if (r.type === 'acctax') {
          ...
          this.apiService.get_url(url, 'hightax').subscribe((responseAcctax: Hightax) => {
            if (responseAcctax.kingdom) {
              r.taxa = responseAcctax.kingdom + ' > ' + responseAcctax.phylum + ' > ' + responseAcctax.taxclass + ' > ' +
                       responseAcctax.taxorder + ' > ' + family + ' > ' + sname;
            } else {
              r.taxa = 'community';
            }
